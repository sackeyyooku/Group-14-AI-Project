...
st.set_page_config(
   page_title="GROUP 14 MOVIE RECOMMENDATION SYSTEM",
)

def run():
    img1 = Image.open('./meta/cinema.jpg')
    img1 = img1.resize((500,300),)
    st.image(img1,use_column_width=False)
    st.title("GROUP 14 MOVIE RECOMMENDATION SYSTEM")
    st.markdown('''<h4 style='text-align: center; color: #ebe41e;'>* Data is based "IMDB 5000 Movie Dataset"</h4>''',
                unsafe_allow_html=True)
    genres = ['Action', 'Adventure', 'Animation', 'Biography', 'Comedy', 'Crime', 'Documentary', 'Drama', 'Family',
              'Fantasy', 'Film-Noir', 'Game-Show', 'History', 'Horror', 'Music', 'Musical', 'Mystery', 'News',
              'Reality-TV', 'Romance', 'Sci-Fi', 'Short', 'Sport', 'Thriller', 'War', 'Western']
    movies = [title[0] for title in movie_titles]
    category = ['--Select category of recommendation--', 'Movie based', 'Genre based']
